...
lambdaChosen = arrivalRate(1);                                                                                                              %%%%%%%%% control arrival rate here!
...
% a = 0;
% b = 1;
% rng(0); % Set the seed to 0 (change when not testing)
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%    Scenario #2a - CSMA/CA w/ no hidden terminals w/VCS %%%%%%%%%%%%%
NAV = TRS + CTS + transTimeInSlots + sifsDur + ACK; % NAV duraton in slots 
...
while strcmp(lonelyStation,'Complete') ~=1 
   ...
   if length(arrivalsSlottedA) >= frameTrackerA && length(arrivalsSlottedB) >= frameTrackerB
        % aquire station data
        [currentArrival_A, currentArrival_B,endOfBackoff_A, endOfBackoff_B, endNav_A, endNav_B] = probeBufferTops(arrivalsSlottedA, arrivalsSlottedB,frameTrackerA, frameTrackerB, currentBackoffA, currentBackoffB, difsDur, NAV);
   end
   ...
   if frameTrackerA > length(arrivalsSlottedA) % stations A is done
       [lonelyStation, transmissionLog] = stationA_IsDone(arrivalsSlottedB, frameTrackerB, currentBackoffB, transmissionLog, difsDur, endOfBackoff_A, NAV,transTimeInSlots, slotDuration, CW_current, frameTrackerA, countDownWinnerA, countDownWinnerB);

   elseif frameTrackerB > length(arrivalsSlottedB) % stations B is done
       [lonelyStation,transmissionLog] = stationB_IsDone(arrivalsSlottedA, frameTrackerA, currentBackoffA, transmissionLog, difsDur,endOfBackoff_B, NAV,transTimeInSlots, slotDuration, CW_current, frameTrackerB, countDownWinnerA, countDownWinnerB);
...
writeDataToFile(lambdaChosen, transmissionLog,dataFrameBits, numCollisions)
...
function plotContentionPeriod(k, sifsDur, ACK, difsDur, transmissionLog)
    ...
    if  (strcmp(transmissionLog{k, 5}, 'COLLISION'))
        ...
        plot([beginTransmitSlot, startBackoff], [y,y], 'Color', 'cyan', 'LineWidth', 4)
...
function [lonelyStation, transmissionLog] = stationA_IsDone(arrivalsSlottedB, frameTrackerB, currentBackoffB, transmissionLog, difsDur, endOfBackoff_A, NAV, transTimeInSlots, slotDuration, CW_current, frameTrackerA, countDownWinnerA, countDownWinnerB)
...
function [lonelyStation, transmissionLog] = stationB_IsDone(arrivalsSlottedA, frameTrackerA, currentBackoffA, transmissionLog, difsDur, endOfBackoff_B,NAV, transTimeInSlots, slotDuration, CW_current, frameTrackerB, countDownWinnerA, countDownWinnerB)
...
function [currentArrival_A, currentArrival_B,endOfBackoff_A, endOfBackoff_B, endNav_A, endNav_B] = probeBufferTops(arrivalsSlottedA, arrivalsSlottedB, frameTrackerA, frameTrackerB, currentBackoffA, currentBackoffB, difsDur, NAV)
...
function writeDataToFile(lambdaChosen, transmissionLog,dataFrameBits, numCollisions)
    ...
    throughput = (length(transmissionLog(:,1)) - 1 - numCollisions) * dataFrameBits/ totalSimulationTime; 
    % determine fairness
    ...
    fprintf(fileID, 'lambda:,  throughput:, collisions: , Node A Bias backoffRace, Node A Bias runClip:\n');
    fprintf(fileID, '%d,  %d,  %d,  %d, %d\n', lambdaChosen, throughput, numCollisions, biasTowardA, biasTowardAClip );
    fclose('all');
end