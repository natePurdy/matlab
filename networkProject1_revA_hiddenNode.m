...
for i=1:length(arrivalRate)
    ...
    vcsFlag = 1; % ---------------------------------------------------                USE Virtual carrier sensing
    a = 0;
    b = 1;
    ...
    %%
    %%%%%%%%%%%%%%%%%%%%%%%%%    Scenario #2b - CSMA/CA hidden node %%%%%%%%%%%%%
    if vcsFlag ==0 % scenario 2b: CSMA/CA single domain with VCS (similar but w/ RTS and CTS before transmission
        NAV = sifsDur*2+ CTS + transTimeInSlots + sifsDur + ACK; % w/VCS
    else
        NAV = transTimeInSlots + sifsDur + ACK; % NAV duraton in slots (no RTS/CTS if no VCS - )
    end
    % strt with lambda = 100 frames / sec
    currentSlot = 0;
    numCollisionsA = 0;
    numCollisionsB = 0; % account for A and B experiencing different number of collisions
    attemptsToTransmitA = 0;
    attemptsToTransmitB = 0;
    frameTrackerA = 1;
    frameTrackerB = 1;
    remainingBackoff = 0;
    ...
    hold on;
    debug = 0;
    currentBackoffA = 0; currentBackoffB = 0;
    ...
    while strcmp(lonelyStation,'Complete') ~=1 
        ...
        CW_current = CW(cw);
        % rng(debug)
        if strcmp(hasRemainingBackoff, 'none')
            currentBackoffA = randi([0 CW_current-1]);
            currentBackoffB = randi([0 CW_current-1]);
        end
        if hasRemainingBackoff == 'A'
            currentBackoffA = remainingBackoff;
            currentBackoffB = randi([0 CW_current-1]);
        end
       if hasRemainingBackoff == 'B'
            currentBackoffB = remainingBackoff;
       ...
       % least one frame to transmit, see whats up with the arrivals (probing
       % buffers same between vcs and non-vcs versions)
       if length(arrivalsSlottedA) >= frameTrackerA && length(arrivalsSlottedB) >= frameTrackerB
            % aquire station data
            ...
            if vcsFlag ==1 
                endCTS_A = endOfBackoff_A + sifsDur + RTS + CTS; % use for finding collision event during VCS
                endCTS_B = endOfBackoff_B + sifsDur + RTS + CTS;
            end
       end
    
       % first check to see if one of the stations is done stransmitting (
       % same for vcs and no-vcs)
       ...
       % case: vcs collision: RTS OR CTS collision
       elseif vcsFlag ==1 && ((endOfBackoff_B >=endOfBackoff_A) && (endOfBackoff_B <= endCTS_A)...
               || (endOfBackoff_A >= endOfBackoff_B) && (endOfBackoff_A <= endCTS_B))
           % neither will recieve a CTS, retry at end of RTS-CTS window
            numCollisionsA = numCollisionsA + 1;
            numCollisionsB = numCollisionsB + 1;
            ...
            if vcsFlag ==1 % with vcs, collision is realized when no CTS recieved by sender
                endOfContentionPeriodA = endOfBackoff_A + RTS + CTS + sifsDur*2; % advance to end of not CTS
                endOfContentionPeriodB = endOfBackoff_B + RTS + CTS + sifsDur*2;
            else
                endOfContentionPeriodA = endOfBackoff_A + NAV;
                endOfContentionPeriodB = endOfBackoff_B + NAV;
            end
            endOfContentionPeriod = min([endOfContentionPeriodA,endOfContentionPeriodB]);
            ...
            arrivalsSlottedA(frameTrackerA) = endOfContentionPeriodA;
            arrivalsSlottedB(frameTrackerB) = endOfContentionPeriodB;               
    
    
       % case (non vcs): COLLISION FOR ANY DATA OVERLAP (NAV to NAV ragion comparsion)
       % possible collision if using VCS... dig into it
       elseif  (vcsFlag == 0) && ((endOfBackoff_B < endOfBackoff_A) &&  (endNav_B > endOfBackoff_A) || ...
               (endOfBackoff_B < endNav_A) && (endNav_B > endNav_A) || (endOfBackoff_B == endOfBackoff_A) && (endNav_A == endNav_B))
            ...
            if vcsFlag ==1 % with vcs, collision is realized when no CTS recieved by sender
                endOfContentionPeriodA = endOfBackoff_A + RTS + CTS + sifsDur*2; % both have same backoff end point at this point
                endOfContentionPeriodB = endOfBackoff_B + RTS + CTS + sifsDur*2;
            else
                endOfContentionPeriodA = endOfBackoff_A + NAV;
                endOfContentionPeriodB = endOfBackoff_B + NAV;
       ...
       % case: one arrives during NAV from CTS of AP
        % node B has arrivals after end of CTS of NODE A - defer 
       elseif  vcsFlag ==1 && (currentArrival_B > endCTS_A && currentArrival_B < endNav_A) % case: A is first, B arrival during A's transmission, defer B to end of NAV
           % node A is up
            winner = 'Node_A';
            ...
            % remainingBackoff = currentBackoffB;
        ...
        % case 2: Node A has arrival after end of CTS of NODE B - defer A
       elseif vcsFlag ==1 && (currentArrival_A > endCTS_B && currentArrival_A < endNav_B)
            % node B is up
            winner = 'Node_B';  
            ...
            % remainingBackoff = currentBackoffA;
     ...
     % case: Simple case: No contention necassary since no overlap
     %B plans to transmit after A anyways
       elseif endOfBackoff_B >= endNav_A 
            % node A is up
            winner = 'Node_A';
            % log events
            endOfContentionPeriod = endNav_A;
            ...
            hasRemainingBackoff = 'B';
            remainingBackoff = currentBackoffB;
            attemptsToTransmitB = attemptsToTransmitB - 1; % decerement attempts if one not made (counter the auto inc outside this loop)
       % A plans to transmit after B anyways, dont process A this round
       elseif endOfBackoff_A >= endNav_B
            % node B is up
            winner = 'Node_B';
            endOfContentionPeriod = endNav_B;
            frameTrackerB = frameTrackerB + 1; % process next frame next time arounD        
            transmissionLog = [transmissionLog; {endOfBackoff_B, endOfBackoff_B + transTimeInSlots, endOfBackoff_B-currentBackoffB, endOfContentionPeriod, 'Node_B', currentArrival_B, endOfContentionPeriod*slotDuration, CW_current, frameTrackerA, frameTrackerB, countDownWinnerA, countDownWinnerB}];
            attemptsToTransmitA = attemptsToTransmitA - 1;
            hasRemainingBackoff = 'A';
            remainingBackoff = currentBackoffA;
       ...
       % the contention
        elseif currentArrival_B >= currentArrival_A && currentArrival_B <= endOfBackoff_A || ...
                currentArrival_A >= currentArrival_B && currentArrival_A <= endOfBackoff_B   
            ...
            elseif endOfBackoff_B < endOfBackoff_A    % B wins the backoff counter
                % node B is up
                winner = 'Node_B';
                endOfContentionPeriod = endNav_B;
                frameTrackerB = frameTrackerB + 1; % process next frame next time arounD
            ...
            % COLLISION IF RACE CONDITION WAS NOT MET TO DECLARE WINNER
            numCollisionsA = numCollisionsA + 1;
            numCollisionsB = numCollisionsB + 1;
            % double the contention window
            if CW_current < 1024
        ...
        % hack on to align queue of frames (keep sequential nature of data
        % true)
        ...
        if transmissionLog{end,7} > 9.999 % sim force kill after 10 sec
            lonelyStation='Complete';
        end
        attemptsToTransmitA = attemptsToTransmitA + 1;
        attemptsToTransmitB = attemptsToTransmitB + 1;
        
    end

    % % PLOT SIM RESULT TIMELINE (MESSY BUT USEFUL)
    % for ii=2:length(transmissionLog(:,1))
    %     plotContentionPeriod(i, sifsDur, ACK, difsDur, transmissionLog)
    % end

    
    % WRITE DATA TO TEXT FILE
    writeDataToFile(lambdaChosen, transmissionLog,dataFrameBits, numCollisionsA, numCollisionsB, attemptsToTransmitA,attemptsToTransmitB, vcsFlag, frameTrackerA, frameTrackerB)
end
%% 
...
function plotContentionPeriod(k, sifsDur, ACK, difsDur, transmissionLog)
    ...
    if k == length(transmissionLog(:,1))% note the different colors
        ...
        % Add legend entries
        legend([hA, hB, hBackOff], {'Node A', 'Node B', 'BACKOFF'})
...
function [lonelyStation, transmissionLog] = stationB_IsDone...
    (arrivalsSlottedA, frameTrackerA, currentBackoffA, transmissionLog, difsDur, endOfBackoff_B,NAV, transTimeInSlots, slotDuration, CW_current, frameTrackerB, countDownWinnerA, countDownWinnerB, vcsFlag, RTS)
        ...
        for i=0:length(arrivalsSlottedA) - frameTrackerA %stupid matlab
            ...
            if vcsFlag ==1
                endNav_A = endOfBackoff_A + NAV + RTS; % account for RTS     
            else
...
function writeDataToFile(lambdaChosen, transmissionLog,dataFrameBits, numCollisionsA, numCollisionsB, attemptsToTransmitA, attemptsToTransmitB, vcsFlag, frameTrackerA, frameTrackerB)
    ...
    if vcsFlag == 1
        fileID = fopen('HIddenDomain_withVCS.txt', 'a');
    else
        fileID = fopen('HiddenDomain.txt', 'a');
    end
    % Write values and a newline character
    fprintf(fileID, 'lambda:,  throughputA:, throughputB, collisionsA, collsionsB: , attemptsToTransmitA, attemptsToTransmitB, Node A Bias backoffRace, Node A Bias runClip:\n');
    fprintf(fileID, '%d,  %d,  %d, %d, %d, %d, %d, %d, %d\n', lambdaChosen, throughputA, throughputB, numCollisionsA, numCollisionsB, attemptsToTransmitA, attemptsToTransmitB, biasTowardA, biasTowardAClip );
    fclose('all');
end
